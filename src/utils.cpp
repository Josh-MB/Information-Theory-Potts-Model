...
{
	if ((u + 1) % std::max<size_t>(1, (U / 10)) == 0) fmt::print("{} : {}% complete\n", msg, 10 * (u + 1) / std::max<size_t>(1, (U / 10))); // report progress of iterative sim from 10% - 100%
}

void progrep(FILE * fp, const char* const msg, const size_t u, const size_t U)
{
	if ((u + 1) % std::max<size_t>(1, (U / 10)) == 0) fmt::print(fp, "{} : {}% complete\n", msg, 10 * (u + 1) / std::max<size_t>(1, (U / 10))); // report progress of iterative sim from 10% - 100%
}

void print_debug_info()
{
	fmt::print("Num States: {:d}\n", numStates);
...
int set_num_threads(int threadLimit, bool verbose)
{
	int num_threads = omp_get_max_threads();
	if (threadLimit > 0) {
		omp_set_num_threads(threadLimit);
		num_threads = threadLimit;//omp_get_num_threads();
	}
	else {
		omp_set_num_threads(num_threads);
	}
	if (verbose) {
		fmt::print("Using {} threads (Max available: {})\n", num_threads, omp_get_max_threads());
	}
	return num_threads;
}
